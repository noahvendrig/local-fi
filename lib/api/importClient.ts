...
export async function fetchImportJobs(limit = 20): Promise<ImportJob[]> {
  const res = await fetch(apiUrl(`/api/v1/import/jobs?limit=${limit}`), { headers: authHeaders() });
...
export async function fetchImportJob(jobId: number): Promise<ImportJobWithFiles> {
  const res = await fetch(apiUrl(`/api/v1/import/jobs/${jobId}`), { headers: authHeaders() });
...
export async function cancelImportJob(jobId: number): Promise<ImportJobWithFiles> {
  const res = await fetch(apiUrl(`/api/v1/import/jobs/${jobId}/cancel`), { method: "POST", headers: authHeaders() });
