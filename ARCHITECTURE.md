...
On process startup, any `import_job_files` row left in a non-terminal status (`queued`, or caught mid-pipeline) from a previous crash is reset to `queued` and its job re-enqueued — already-`done` files are untouched, and `spotify_import`'s duplicate-skip check (source provider + URL match) means a track that finished downloading before the crash won't be redownloaded. Anything left in `staging/` older than 24h is swept. (Earlier version of this doc: no auto-resume, just mark the job `failed` — fine for a plain file drop where re-dropping is cheap, but not for a `spotify_import` job where "just redo it" means re-matching and re-downloading a whole playlist.)
...
- **Enforcement, correctly shaped around the `proxy.ts` body constraint**: on an unauthorized request, `proxy.ts` does **not** attempt to return a JSON body itself. Instead it **rewrites** the request to an internal `app/api/unauthorized/route.ts` Route Handler, which — as an ordinary route handler, not a proxy — is free to return the actual `401 { error: { code: 'unauthorized', message } }` JSON body. On a successful check, `proxy.ts` calls `NextResponse.next()` and the request proceeds to its real route handler unmodified. This keeps enforcement centralized (no individual route handler has to remember to check auth — a common real-world source of security bugs) while staying inside what `proxy.ts` is actually allowed to do. (Note: the rewrite target must **not** use a `_`-prefixed folder name — Next.js treats `_folder` as a private, non-routable folder per its project-structure convention, so `app/api/_unauthorized/` 404s instead of resolving. `unauthorized`, without the underscore, is what actually works.)
